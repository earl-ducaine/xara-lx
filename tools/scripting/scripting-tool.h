...
public:
  ...
  virtual void DragPointerMove(DocCoord PointerPos, ClickModifiers ClickMods,
			       Spread* spread, BOOL bSolidDrag);
...
private:
  ...
  DocCoord StartPoint;
  Spread* StartSpread;
  // So that we don't update too often.
  DocCoord PreviousPoint;
  // Is the drag adjust or not?
  BOOL AdjustDrag;
  // Points back to the bezier tool
  BezierTool* BezToolPtr;
  // which modifiers were in evidence from the original click
  ClickModifiers Mods;
  // This op is not re-entrant.
  static BOOL	DragUnderway;
  // Normal freehand cursor
...
private:
  ...
  // Pop our cursor off the stack
  void	PopCursor();
  	// Restores the selection blobs to pre-drag state
...
>	class OpRemoveFloater : public UndoableOperation
	...
	Purpose:        This is the operation that the line tool uses to
	                remove the floating endpoint in an undoable
	                kind of way.
****************************************************************************/
class OpRemoveFloater : public UndoableOperation
{
...
>	class OpInsertFloater : public UndoableOperation
	...
	Created:	06/12/94
	Purpose:        This is the operation that the line tool uses to
			insert the floating endpoint in an undoable
			kind of way.
***************************************************************************/
class OpInsertFloater : public UndoableOperation
{
...
>	class InsertFloaterParam : public OpParam
	...
	Author:		Peter_Arnold (Xara Group Ltd) <camelotdev@xara.com>
	Created:	06/12/94
	Purpose:        This is the OpParam object passed to OpInsertFloater
			to position the floating endpoint
***************************************************************************/
class InsertFloaterParam : public OpParam
{
...
public:
  InsertFloaterParam(DocCoord* pInsertPos, Spread* pInsertSpread,
...
>	class InsertFloaterAction : public Action
	...
	Purpose:        An action to provide the Line tool with an undoable
			way of inserting the floating endpoint.  This
			isn't stored in the document, which makes life
			tricky.
	SeeAlso:
***************************************************************************/
class InsertFloaterAction : public Action
...
>	class RemoveFloaterAction : public Action
	...
	Created:	05/12/94
	Purpose:        An action to provide the Line tool with an undoable
			way of removing the floating endpoint.  This
			isn't stored in the document, which makes life
			tricky.
	SeeAlso:
****************************************************************************/
