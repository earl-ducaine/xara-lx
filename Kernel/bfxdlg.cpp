...
// $Id: bfxdlg.cpp 1282 2006-06-09 09:46:49Z alex $
...
BOOL BfxDlg::OriginalChanged()
{
    ...
    BROADCAST_TO_CLASS(DialogMsg(WindowID,
                                 DIM_OUT_OF_SPACE,
                                 _R(IDC_BFXDLG_ORIGREDRAW),
                                 0,
                                 OpenPage),
                       DialogOp);  // Pretty horrible bodge
...
BOOL BfxDlg::RenderBitmap(ReDrawInfoType* ExtraInfo, KernelBitmap * BitmapToUse)
{
    ...
    if (pRender!=NULL)
    {
        ...
        if (BitmapToUse)
        {
            ...
            INT32 bitmap_size_low_x;
            INT32 bitmap_size_low_y;
            INT32 bitmap_size_high_x;
            INT32 bitmap_size_high_y;
            ...
            if (!((pNodeBitmap == NULL) || (!pNodeBitmap->SetUpPath(12,12))))
            {
                ...
                if (BMAspect>RedrawAspect)
                {
                    ...
                    bitmap_size_low_y = (VirtualSize.Height() - NewHeight)/2;
                    bitmap_size_high_y = bitmap_size_low_y + NewHeight;
                }
                else
                {
                    ...
                    bitmap_size_low_x = (VirtualSize.Width() - NewWidth)/2;
                    bitmap_size_high_x = bitmap_size_low_x + NewWidth;
                }
...
MsgResult BfxDlg::Message( Msg* Message)
{
    if (IS_OUR_DIALOG_MSG(Message))
    {
        ...
        if (ThePage)
        {
            ...
            if (ThePage == _R(IDD_BFXDLG_PG1))
            {
                HandleFlipAndRotateMsg(Msg, &CommitValues, &EndDialog);
            } else if (ThePage == _R(IDD_BFXDLG_PG2))
            {
                HandleResizeMsg(Msg, &CommitValues, &EndDialog);
            } else if (ThePage == _R(IDD_BFXDLG_PG3))
            {
                HandleBrightnessAndContrastMsg(Msg, &CommitValues, &EndDialog);
            } else if (ThePage == _R(IDD_BFXDLG_PG4))
            {
                HandleDitherMsg(Msg, &CommitValues, &EndDialog);
            } else if (ThePage == _R(IDD_BFXDLG_PG5))
            {
                HandleSpecialEffectsMsg(Msg, &CommitValues, &EndDialog);
            } else if (ThePage == _R(IDD_BFXDLG_PG6))
            {
                HandleAlchemistMsg(Msg, &CommitValues, &EndDialog);
            } else
            {
        ...
        if (EndDialog)  // Dialog communication over
        {
            if (pOriginal && CommitValues)
            {
                ...
                OpDescriptor* pOpDesc = OpDescriptor::FindOpDescriptor(CC_RUNTIME_CLASS(BfxOp));
                ERROR3IF_PF(pOpDesc == NULL, (_T("Couldn't find OPTOKEN_OPBFXOP op descriptor")));
...
void BfxDlg::HandleFlipAndRotateMsg(DialogMsg* Msg, BOOL *pCommitValues, BOOL *pEndDialog)
{
    ...
    case DIM_SELECTION_CHANGED:
    {
        ...
        if (Msg->GadgetID == _R(IDC_BFXDLG_FLIPX) ||
            Msg->GadgetID == _R(IDC_BFXDLG_FLIPX) ||
...
void BfxDlg::Do(OpDescriptor*)
{
    ...
    SetStringGadgetValue(_R(IDOK), _R(IDS_BFXDLG_INSERT));
    SetStringGadgetValue(_R(IDCANCEL), _R(IDS_BFXDLG_CLOSE));
}
