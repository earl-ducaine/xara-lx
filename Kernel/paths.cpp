...
void Path::DrawControlLine(RenderRegion* pRender, const DocCoord& Start, const DocCoord& End)
{
    // Maybe rather than force redraw, which syncronously causes a redraw to take place,
    // while we might be in the middle of one right now, instead ivalidate the view, which
    // will cause a paint event?
