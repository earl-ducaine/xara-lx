...
// $Id: bfxdlg2.cpp 1282 2006-06-09 09:46:49Z alex $
...
void BfxPlugInDlg::Remove24BitTempCopy()
{
	...
	std::list<String_256>::iterator Pos;
	...
	while(!m_p24BitCopyNames.empty())
	{
		m_p24BitCopyNames.pop_front();
	}
}
...
//	pOriginal = NULL;
	...
	if (m_pParam != 0)
	{
		pOriginal = (KernelBitmap*)((void*)m_pParam->Param1);
	...
	if(pOriginal != NULL)
	{
		...
		if(pOriginal->ActualBitmap->GetBitmapInfoHeader()->biBitCount == 32)
		{
			...
			else
			{
				...
				NewName += _T("(24-bit Copy)");

				// Add the name to our copy name list
				m_p24BitCopyNames.push_back(NewName);
...
BOOL BfxPlugInDlg::FixOriginalComboBox()
{
	...
	String_256 Str;
    String_256 the_item_name;
    INT32 the_item_id;
    KernelBitmap* the_item_kernel_bitmap_ptr;
	...
	//				SetStringGadgetValue(_R(IDC_EFFECT),&Str,FALSE, Index);
					...
					the_item_kernel_bitmap_ptr = (KernelBitmap*)bitmap;
	...
	if (pRender!=NULL)
	{
		...
		if (BitmapToUse)
		{
			...
			if (!((pNodeBitmap == NULL) || (!pNodeBitmap->SetUpPath(12,12))))
			{
                ...
                INT32 bitmap_size_low_corner_x;
                INT32 bitmap_size_low_corner_y;
                INT32 bitmap_size_high_corner_x;
                INT32 bitmap_size_high_corner_y;
				...
				if (BMAspect>RedrawAspect)
				{
					...
					bitmap_size_low_corner_y = (VirtualSize.Height() - NewHeight)/2;
					bitmap_size_high_corner_y = bitmap_size_low_corner_y + NewHeight;
				}
				else
				{
					...
					bitmap_size_low_corner_x = (VirtualSize.Width()-NewWidth)/2;
					bitmap_size_high_corner_x = bitmap_size_low_corner_x + NewWidth;
				}

				// And set this in our bitmap node
                DocRect new_bitmap_size(bitmap_size_low_corner_x,
                                        bitmap_size_low_corner_y,
                                        bitmap_size_high_corner_x,
                                        bitmap_size_high_corner_y);

				pNodeBitmap->CreateShape(new_bitmap_size);
...
MsgResult BfxPlugInDlg::Message( Msg* Message)
{
	if (IS_OUR_DIALOG_MSG(Message))
	{
        ...
        case DIM_REDRAW:
            ...
            else if (Msg->GadgetID == _R(IDC_BFXDLG_ORIGREDRAW2))
            {
        ...
        case DIM_SELECTION_CHANGED:
            if (Msg->GadgetID == _R(IDC_BFXDLG_ORIGCOMBO))
            {
		...
		if (EndDialog)	// Dialog communication over
		{
			...
			if (pOriginal != NULL && CommitValues && pDestination != NULL)
			{
				...
				if (pDestBitmap != NULL && pBmpList != NULL)
				{
					...
					if (m_pParam)
						m_pParam->Param1 = (void*)pDestBitmap;
				}
				else
...
void BfxPlugInDlg::HandleFlipAndRotateMsg(DialogMsg* Msg, BOOL *pCommitValues, BOOL *pEndDialog)
{
    ...
    case DIM_SELECTION_CHANGED:
    {
        ...
        if ((Msg->GadgetID == _R(IDC_BFXDLG_FLIPX)) ||
            (Msg->GadgetID == _R(IDC_BFXDLG_FLIPY)) ||
            (Msg->GadgetID == _R(IDC_BFXDLG_ROTATE90)) ||
            (Msg->GadgetID == _R(IDC_BFXDLG_ROTATE180)) ||
            (Msg->GadgetID == _R(IDC_BFXDLG_ROTATE270)))
        {
            SetDirty();
        }
...
void BfxPlugInDlg::HandleResizeMsg(DialogMsg* Msg, BOOL *pCommitValues, BOOL *pEndDialog)
{
    ...
    case DIM_TEXT_CHANGED:	 		   // Text changed
    {
        ...
        {
            if (Msg->GadgetID == _R(IDC_BFXDLG_NEWXSIZE))
            {
                if (pOrigInfo->PixelWidth)
                {
                    ReEntered = TRUE;
                    SetLongGadgetValue(_R(IDC_BFXDLG_NEWYSIZE),
                                       (INT32)(0.5 +
                                               (((double)(pOrigInfo->PixelHeight))/
                                                ((double)(pOrigInfo->PixelWidth))*((double)(NewX)) )));
                    ReEntered = FALSE;
                }
            }
            else if (Msg->GadgetID == _R(IDC_BFXDLG_NEWYSIZE))
            {
                if (pOrigInfo->PixelHeight)
                {
                    ReEntered = TRUE;
                    SetLongGadgetValue(_R(IDC_BFXDLG_NEWXSIZE),
                                       (INT32)(0.5 +
                                               (((double)(pOrigInfo->PixelWidth))/
                                                ((double)(pOrigInfo->PixelHeight))*((double)(NewY)))));
                    ReEntered = FALSE;
                }
            }
        }
...
//		case DIM_SLIDER_POS_IDLE:
    ...
    case DIM_SELECTION_CHANGED:
        ...
        if (Msg->GadgetID == _R(IDC_BFXDLG_BRIGHTNESS) ||
            Msg->GadgetID == _R(IDC_BFXDLG_CONTRAST) ||
            Msg->GadgetID == _R(IDC_BFXDLG_COLOUR))
        {
            SetDirty();
        }
        break;
...
//EnableGadget(_R(IDC_BFXDLG_OCTREECOLOUR), FALSE); // Accusoft library crashes
    ...
    case DIM_SELECTION_CHANGED:
        ...
        if (Msg->GadgetID == _R(IDC_BFXDLG_BAYERMONO) ||
            Msg->GadgetID == _R(IDC_BFXDLG_HALFTONEMONO) ||
            Msg->GadgetID == _R(IDC_BFXDLG_DIFFUSIONMONO) ||
            Msg->GadgetID ==  _R(IDC_BFXDLG_BAYERCOLOUR) ||
            Msg->GadgetID ==  _R(IDC_BFXDLG_DIFFUSIONCOLOUR) ||
            Msg->GadgetID ==  _R(IDC_BFXDLG_POPULARITY) ||
            Msg->GadgetID ==  _R(IDC_BFXDLG_MAKEGREYSCALE) ||
            Msg->GadgetID ==  _R(IDC_BFXDLG_OCTREECOLOUR))
        {
            SetDirty();
        }
        *pCommitValues = TRUE;
    ...
    case DIM_SOFT_COMMIT:
    case DIM_COMMIT:
        if (GetBoolGadgetSelected(_R(IDC_BFXDLG_BAYERMONO)))
            SetEffect(CC_RUNTIME_CLASS(ABFXBayerMono));
        if (GetBoolGadgetSelected(_R(IDC_BFXDLG_HALFTONEMONO)))
            SetEffect(CC_RUNTIME_CLASS(ABFXHalftoneMono));
        if (GetBoolGadgetSelected(_R(IDC_BFXDLG_DIFFUSIONMONO)))
            SetEffect(CC_RUNTIME_CLASS(ABFXDiffusionMono));
        if (GetBoolGadgetSelected(_R(IDC_BFXDLG_BAYERCOLOUR)))
            SetEffect(CC_RUNTIME_CLASS(ABFXBayerColour));
        if (GetBoolGadgetSelected(_R(IDC_BFXDLG_DIFFUSIONCOLOUR)))
            SetEffect(CC_RUNTIME_CLASS(ABFXDiffusionColour));
        if (GetBoolGadgetSelected(_R(IDC_BFXDLG_POPULARITY)))
            SetEffect(CC_RUNTIME_CLASS(ABFXPopularityColour));
        if (GetBoolGadgetSelected(_R(IDC_BFXDLG_MAKEGREYSCALE)))
            SetEffect(CC_RUNTIME_CLASS(ABFXMakeGreyscale));
        if (GetBoolGadgetSelected(_R(IDC_BFXDLG_OCTREECOLOUR)))
            SetEffect(CC_RUNTIME_CLASS(ABFXOctree));
        break;
    default:
        break;
	}
...
BOOL BfxPlugInDlg::Init()
{
	GetApplication()->DeclareSection(_T("Bitmap Effects"), 1);
	GetApplication()->DeclarePref(_T("Bitmap Effects"),
                                  _T("Interactive Brightness & Contrast"),
                                  &InteractiveBCC,
                                  FALSE,
                                  TRUE);
...
void BfxPlugInDlg::DoWithParam(OpDescriptor* pOp, OpParam* pParam)
{
	...
	if (pParam)
	{
		...
		m_pDocument = (Document*)((void*)pParam->Param2);
	}
