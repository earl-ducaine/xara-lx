...
PORTNOTE("other", "Remove XPE hooks")
                ...
                // EED -- this is the other (and more likely) entry point for the effects
                // dialog, i.e. as oposed to the PHOTOSHOPPLUGINS section.
                ...
                else if (Msg->GadgetID == _R(IDC_BMPGAL_PLUGINS))
                {
                    // Obtain a pointer to the op descriptor for the create operation
                    OpDescriptor* OpDesc =
                        OpDescriptor::FindOpDescriptor(CC_RUNTIME_CLASS(BfxDlg));
                    ...
                    {
                        OpParam param((void*)SelectedBitmap, (void*)NULL);
                        OpDesc->Invoke(&param);
                    }
                    else
                    {
                        BROADCAST_TO_CLASS(TraceMsg(SelectedBitmap), DialogOp);
                    }
                    return SuperGallery::Message(Message);
                }
...
// Only add in if required - general plug-in removal at present
        ...
        ok = ok && BfxPlugInContextMenu::BuildMenu(pSelectedBitmap, pDocument, TheMenu, NULL, TRUE);
