...
// $Id: tracedlg.cpp 1334 2006-06-17 09:43:37Z alex $
...
BOOL TraceDlg::Init()
{
    ...
    GetApplication()->DeclarePref(_T("Tracer"),
                                  _T("DefaultMode"),
                                  &DefaultMode,
                                  0,
                                  ((INT32)TRACEMETHOD_ILLEGAL) - 1);

    String_256 PrefName;

    // not resourced as .ini file setting
    String_256 PrefBase(_T("Tracer mode #1%d setting #2%d"));

    for (Mode = 0; Mode < (INT32)NUM_TRACEMETHOD; Mode++)
    {
        for (Setting = 0; Setting < (INT32)NUM_TRACESETTING; Setting++)
        {
            PrefName._MakeMsg(PrefBase, Mode, Setting);
            GetApplication()->DeclarePref(_T("Tracer"),
                                          PrefName,
                                          &(DefaultSetting[Mode][Setting]),
                                          0,
                                          100);
        }
    }
