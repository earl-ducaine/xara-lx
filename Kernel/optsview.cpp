...
bool ViewTab::CommitBlobSizesPrefs(CGadgetID gadget, LPCTCHAR section, LPCTCHAR pref)
{
    BOOL is_valid = false;
    int value = pPrefsDlg->GetLongGadgetValue(gadget,
                                              1,
                                              255,
    ...
    if (is_valid)
    {
        BOOL set_ok = Camelot.SetPrefValue(section,
                                     pref,
                                     &value);
        ERROR2IF(!set_ok, 2, _R(IDE_OPTS_SETPREF_VIEW));
    }
    else
    {
        // Invalid value entered by user, who has already been warned.
        return FALSE;
    }
}
...
//      ERROR2(FALSE,_R(IDE_OPTS_READPREF_VIEW));
    ...
    bool success = CommitBlobSizesPrefs(_R(IDC_OPTS_SELECTED_CONTOL_POINT_SIZE),
                                        _T("Selection Blob Sizes"),
                                        _T("Selected Blobs"));

    success = CommitBlobSizesPrefs(_R(IDC_OPTS_UNSELECTED_CONTOL_POINT_SIZE),
                                   _T("Selection Blob Sizes"),
                                   _T("UnSelected Blobs")) &&
        success;

    success = CommitBlobSizesPrefs(_R(IDC_OPTS_SELECTED_MULTI_CONTOL_POINT_SIZE),
                                   _T("Selection Blob Sizes"),
                                   _T("Multi Stage Selected Blobs")) &&
        success;

    success = CommitBlobSizesPrefs(_R(IDC_OPTS_UNSELECTED_MULTI_CONTOL_POINT_SIZE),
                                   _T("Selection Blob Sizes"),
    ...
    // Set the document altered state true if any of the document/view based items have
    // been changed but only if we have a valid document and view.
    //
    // ToDo (EED) -- get rid of this someday, it serves no purpose.
    if (pView != NULL && pDocument != NULL)
    {
        //removed 27/8/96 Transparency Control DEAD
        if (OldBackgroundRedraw != BackgroundRedraw)
        {
...
void ViewTab::GetBlobSizesPrefs(CGadgetID gadget, LPCTCHAR section, LPCTCHAR pref)
{
    int control_point_size = 0;

    bool read_ok = Camelot.GetPrefValue(section,
                                        pref,
                                        &control_point_size);
    pPrefsDlg->SetLongGadgetValue(gadget, control_point_size);
    ERROR2IF(!read_ok, FALSE, _R(IDE_OPTS_READPREF_VIEW));
}
...
//  ERROR2IF(!ReadOk,FALSE,_R(IDE_OPTS_READPREF_VIEW));
    ...
    INT32 control_point_size = 0;

    // ToDo (EED) -- As much as possible use camel-case rather than spaces for word
    // delimitation. It makes the preferences file (arguably) more human readable.

    GetBlobSizesPrefs(_R(IDC_OPTS_SELECTED_CONTOL_POINT_SIZE),
                      _T("Selection Blob Sizes"),
                      _T("Selected Blobs"));

    GetBlobSizesPrefs(_R(IDC_OPTS_UNSELECTED_CONTOL_POINT_SIZE),
                      _T("Selection Blob Sizes"),
                      _T("UnSelected Blobs"));

    GetBlobSizesPrefs(_R(IDC_OPTS_SELECTED_MULTI_CONTOL_POINT_SIZE),
                      _T("Selection Blob Sizes"),
                      _T("Multi Stage Selected Blobs"));

    GetBlobSizesPrefs(_R(IDC_OPTS_UNSELECTED_MULTI_CONTOL_POINT_SIZE),
                      _T("Selection Blob Sizes"),
                      _T("Multi Stage UnSelected Blobs"));

    // Set the force JPEG DPI flag
    pPrefsDlg->SetLongGadgetValue(_R(IDC_JPEG_96DPI_CHECK),
                                  INT32(JPEGImportFilter::GetImportAt96dpi()));
