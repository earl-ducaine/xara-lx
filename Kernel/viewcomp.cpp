...
>   BOOL ViewRecordHandler::HandleDocumentViewRecord(CXaraFileRecord* pCXaraFileRecord)
    ...
    Purpose:    Handles the document view definition record. All this does is set the
                current view quality (applied to subsequent view port & doc view records)
...
BOOL ViewRecordHandler::HandleDocumentViewRecord(CXaraFileRecord* pCXaraFileRecord)
{
    ...
    // then apply the data. Note, at this point we have created a new document for our
    // view record, but we haven't updated anything in the 'ui' i.e. DocView.
    if (ok && pView && ViewCount == 1 && Restore)
    {
        // Set the view for the following code. The zoom ops work on the current view, as
        // do the coordinate conversion functions.
        //
        // Note, the following (wrong) code seems to have been the result of a
        // misunderstanding about the order of events when the view for an existing
        // document is set up. When opening a *.xar file, first a new document is created
        // for it (wxDocManager::CreateDocument) we then go through a process of opening
        // the file, which is an interleaved process of reading various types and
        // dispatching to the proper class (filter) in which to read the specifics of the file.
        //
        // By the time we're here we've read in the file data and have our first coherent
        // chuck of data to create the objects that will ultimately be rendered, but we
        // haven't instatiated those objects and we haven't yet created the UI objects on
        // which they will be displayed, that is an 'asynchronous' asynchronous process
        // that will happen some time in the 'future', su we must becareful not to perform
        // any actions that dependon on things in the DocView singleton (in particular) to
        // have been set up yet.
        //
        // CCamApp::OnRecentFile --> wxDocManager::CreateDocument --> CCamDoc::OnOpenDocument
        //      CCamDoc::DoOpenDocument --> CCamDoc::GeneralOpenDocument --> InvokeNativeFileOp -->
        //      OpDescriptor::Invoke --> OpMenuLoad::DoWithParam --> OpMenuLoad::LoadFile -->
        //      FilterFamily::DoImport --> BaseCamelotFilter::DoImport --> BaseCamelotFilter::ReadFile
        //      CXaraFile::ReadNextRecord --> ViewRecordHandler::HandleRecord -->
        //      ViewRecordHandler::HandleDocumentViewRecord
        //
        // wrong:
        // ERROR3IF(pView != DocView::GetCurrent(),
        //          "ViewRecordHandler::HandleDocumentViewRecord DocView::Current is screwed");

        // Set the view scale and positioning
        //
        // First set the correct view scale factor as otherwise some of the conversions
        // may go wrong
        pView->SetViewScale(ScaleFactor);
        // The top left corner of this rectangle is the same as the value returned by
        // GetScrollOffsets().  First take our saved version which is in SpreadCoords.  I
        // know, I know just imagine DocCoord = SpreadCoord at this point.
