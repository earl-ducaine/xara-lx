...
public:
  // The Constructor of the DialogOp class simply sets the resource ID
  // of the dialog and its modality.
  ...
  // The create method reserves everything needed to guarantee that
  // the dialog will appear (it does not show the dialog).
  virtual BOOL Create();
  // The open method displays the dialog box on the display.
  void Open();
  // The close method removes the dialog from the display but keeps
  // all system resources associated with it. It hides the dialog.
  void Close();
  ...
  BOOL BringToTop();
  // Used to determine if a particular gadget can be ticked or not, it
  // only makes sense to tick buttons at the moment
  ...
  // The DialogOP destructor destroys the instance of the DialogOp and
  // all associated resources. If the dialog was open then it is
  // closed.
  ~DialogOp();
  // The following methods call same named functions defined in the
  // DialogManager.  The parameters of the DialogManager functions are
  // identical except for an additional CWindowID parameter.
  // Methods to aid rendering into the dialogue via render regions
  RenderRegion* CreateGRenderRegion(DocRect* pRequiredSize,
  ...
  // This function should never get called. It is required because
  // DYNCREATE is a bodge
  DialogOp();


  // properties
public:
  // Window associated with dialog
  CWindowID WindowID;
...
public:
  ...
  BOOL SendMessageToControl(OpDescriptor* OpDesc, DialogMsg* Msg,
...
protected:
  ...
  // A pointer to camelot's dialog manager. This is stored so we don't
  ...
  // It can only be set to a non NULL value for Dialogs which can have
  // pages i.e. tabbed dialogs derived from DialogTabOp.
...
public:
  ...
  BOOL IsModal() {
    return (DlgMode == MODAL);
  };
  	// Returns the current page id. For use when in tabbed dialogs mode
  CDlgResID GetCurrentPageID();
  // Setting up and killing timer events for dialog boxes
  UINT32 SetTimer(UINT32 nIDEvent,
		  UINT32 nElapse,
