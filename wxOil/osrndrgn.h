...
// Used for rendering text via GDI (It is a simple cache system).
class OSRRFontInfo
{
 public:
  OSRRFontInfo();
...
class CCAPI OSRenderRegion : public RenderRegion
{
  ...
  // and and equivalent function to do the same, only colour
 ...
 protected:
  ...
  // These functions are called from the RenderRegion base class when
 ...
 protected:
  ...
  // NULL if no old palette
  wxPalette* OldPalette;
  // special Gavin-compatible brush
  GBrush GDrawBrush;

 protected:
  void GetValidPen();
  void GetValidBrush();
  // The position in the polygon array to insert points at and the
  // array
  ...
  // The brush origin
  wxPoint NewBrushOrg;
  // This render regions caps. We keep a cached version here as it can
  // be needed in rendering
  RRCaps Caps;

  // Properties
 protected:
  // NULL, or a pointer to colour separation tables
  BYTE* SepTables;
};
