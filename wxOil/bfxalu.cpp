                              ...
                              String_256 * pName, UINT32 ResID, BOOL DoBodge)
{
    ...
    AccusoftFilters::pfnIMG_bitmap_info(Handle, &width, &height, &bpp);
    if (Make32Bits && bpp != 24)
    {
        result = AccusoftFilters::pfnIMG_delete_bitmap(Handle);
...
// Accusoft Promote_8 from monochrome returns ClrUsed=2 but has a 256 entry colour table. Arrrggghhh!
        // pBBMIpbiClrUsed = pBBMI.biClrImportant = UsedColours;
#endif
        if (pBBMI.biClrUsed == 0)
            // 0 is MS speak for maximum
            pBBMI.biClrUsed = UsedColours;

        if (pBBMI.biClrImportant == 0)
            // 0 is MS speak for maximum
