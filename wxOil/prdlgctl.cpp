...
// $Id: prdlgctl.cpp 1742 2006-09-05 17:25:58Z alex $
...
DEVMODE* CCPrintDialog::GetSystemDevMode(HWND hWnd,LPCTSTR pDriverName,LPCTSTR pDeviceName,LPCTSTR pPortName)
{
    ...
    if (hMod != NULL)
    {
        ...
        if (ExtDeviceMode != NULL)
        {
            ...
            LPSTR pDevice = (LPSTR)CCMalloc(buffer_size);
            LPSTR pPort   = (LPSTR)CCMalloc(buffer_size);

            if (pDeviceName != NULL && pPortName != NULL)
            {
                camStrncpy(pDevice, pDeviceName, buffer_size);
                camStrncpy(pPort, pPortName, buffer_size);
                // Note, camStrncpy should have already add null termination if needed.
                pDevice[buffer_size - 1] = TCHAR('0');
                pPort[buffer_size - 1] = TCHAR('0');
...
INT32 CCPrintToFileDialog::DoModal()
{
    ...
    if (Result == IDOK)
    {
        ...
        FileName[FILENAMEBUFSIZE - 1] = TCHAR('\0');
    }
