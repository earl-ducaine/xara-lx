...
EventNumberToName * DialogEventHandler::pHashEventNumberToName = NULL;
...
void insert_into_hash_table(EventNumberToName* pHashEventNumberToName, int x,
			    const char* chars) {
  wxString text = chars;
  (*pHashEventNumberToName)[x] = text;
}
...
BOOL DialogEventHandler::Init() {
  ...
  pHashEventNumberToName = new EventNumberToName;
  if (!pHashEventNumberToName) {
    return FALSE;
  }
  // Clear our own hash tables
  pHashEventNumberToName->clear();
#if _DEBUG
// # if 0 != wxUSE_UNICODE
// # define HASHEVENTNAME(x) (*pHash)[x] = (EventNameString)camStrdup(L ## #x)
// #else
# define HASHEVENTNAME(x) (insert_into_hash_table(pHashEventNumberToName, x, #x))
// #endif
  ...
  // non-client mouse event are obsolete
  ...
  // Obsolete at least as of wxWidgets 3.0
  ...
  TRACE(_T(">>> Event name %d=%s"),
	wxEVT_COMMAND_BUTTON_CLICKED,
	GetEventName(wxEVT_COMMAND_BUTTON_CLICKED));
  WXTYPE EventNum = wxEVT_COMMAND_BUTTON_CLICKED;
  EventNumberToName::iterator i = pHashEventNumberToName->find(EventNum);
  // const TCHAR * text = (const TCHAR *)((i==pHashEventNumberToName->end())?
  // 				       NULL:
  // 				       i->second);
  //return text?text:DefaultString;
  if (i != pHashEventNumberToName->end()) {
    TRACE(_T("Found text %d=%s"), EventNum, i->second);
  }
...
void DialogEventHandler::DeInit()
{
	...
	if (pHashEventNumberToName)
	{
		...
		while (!pHashEventNumberToName->empty())
		{
			current = pHashEventNumberToName->begin();
			EventNameString s=current->second;
			pHashEventNumberToName->erase(current);
			free((void *)s);
		}
		// for good measure
		pHashEventNumberToName->clear();
		delete (pHashEventNumberToName);
		pHashEventNumberToName=NULL;
	}
}
