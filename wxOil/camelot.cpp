...
void CCamApp::SetupTracing(wxCmdLineParser& parser) {
  ...
  if (Username==_T("")) {
    ...
    TRACEUSER("ALL",
	      "Tracing output where user is %s\n",
	      Username);
  }
  TRACEUSER("ALL",
	    "Memory debugging %d, List debugging %d\n",
...
bool CCamApp::OnInit() {
  ...
  wxString SIname = wxString(".XARA-XTREME-WX-") +
    GetAppName() +
    wxString::Format("-%s.ipc", wxGetUserId());
  wxFileName IPCfn(wxGetHomeDir(), SIname);
